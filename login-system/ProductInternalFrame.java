...
public class ProductInternalFrame extends JPanel {

    private ProductDAO productDAO;
    private JTable table;
    private DefaultTableModel model;
    private JTextField searchField;
    private JTextField nameField, descriptionField, priceField, stockField, supplierField;
    ...
    private JPanel createFormPanel() {
        ...
        stockField = addFormRow(panel, gbc, 3, "Estoque:", new JTextField(10));
        supplierField = addFormRow(panel, gbc, 4, "Fornecedor:", new JTextField(20));
    ...
    private JPanel createTablePanel() {
        ...
        searchField.getDocument().addDocumentListener(new DocumentListener() {
            ...
            private void filter() {
                ...
                else populateTable(productDAO.searchByName(q));
            }
        });
        searchPanel.add(searchField, BorderLayout.CENTER);
        panel.add(searchPanel, BorderLayout.NORTH);

        model = new DefaultTableModel(new String[]{"ID", "Nome", "Descricao", "Preco", "Estoque", "Fornecedor"}, 0) {
        ...
        table.getSelectionModel().addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting() && table.getSelectedRow() != -1) {
                ...
                priceField.setText(String.valueOf(model.getValueAt(table.getSelectedRow(), 3)).replace("R$ ", ""));
                stockField.setText(String.valueOf(model.getValueAt(table.getSelectedRow(), 4)));
                supplierField.setText((String) model.getValueAt(table.getSelectedRow(), 5));
            }
        });
    ...
    private void loadData() {
        populateTable(productDAO.findAll());
    }

    private void populateTable(List<Product> list) {
        model.setRowCount(0);
        for (Product p : list) {
            model.addRow(new Object[]{p.getId(), p.getName(), p.getDescription(),
    ...
    private void save() {
        ...
        if (name.isEmpty()) {
            DarkDialog.showError(this, "Erro", "O nome do produto e obrigatorio.");
            return;
        }
        double price = 0.0;
        int stock = 0;
        ...
        Product p = new Product(name, descriptionField.getText().trim(), price, stock, supplierField.getText().trim());
    ...
    private void update() {
        if (selectedId == -1) {
            DarkDialog.showWarning(this, "Atencao", "Selecione um produto na tabela para atualizar.");
            return;
        }
        String name = nameField.getText().trim();
        if (name.isEmpty()) {
    ...
    private void delete() {
        if (selectedId == -1) {
            DarkDialog.showWarning(this, "Atencao", "Selecione um produto na tabela para excluir.");
            return;
        }
        if (DarkDialog.showConfirm(this, "Confirmar", "Deseja realmente excluir este produto?")) {
            if (productDAO.delete(selectedId)) {
                DarkDialog.showInfo(this, "Sucesso", "Produto excluido com sucesso!");
                clearForm();
                loadData();
            } else {
